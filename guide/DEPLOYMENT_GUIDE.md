...
> [!TIP]
> **RECOMMENDED:** Gunakan script **One-Click Deployment** (`deploy.ps1`) untuk otomatis build & deploy.
> Lihat section [One-Click Deployment](#-one-click-deployment-recommended) di bawah.
...
> [!IMPORTANT]
> **SELALU GUNAKAN `deploy.ps1` untuk deploy!** Script ini sudah dioptimasi untuk skip file-file besar yang jarang berubah.

An optimized PowerShell script `deploy.ps1` automates the build, compress, and upload process.
It **automatically excludes** the entire `assets/images` folder to ensure fast uploads.

```powershell
# Run from CMS directory (D:\02____WORKS\04___Server\Projects\CMS\VISTO\CMS)
./deploy.ps1
```

**What it does:**
1. ✅ Runs `npm run build`
2. ✅ Creates `deploy.tar.gz` (excluding `assets/images` folder)
3. ✅ Uploads archive + api.php + db_ucx_schema.sql to server
4. ✅ Extracts and updates ownership on server
5. ✅ Syncs `db_ucx` database schema

> [!NOTE]
> Folder `assets/images` (flags, user-profiles, SVG illustrations) **tidak perlu diupload ulang** karena jarang berubah. Jika ada perubahan image, upload manual via scp.
...
## 🚀 Smart API Features (2026-02-02)

### 1. Smart Auto-Migration (NEW)
The `api.php` now includes **Automatic Schema Updates**. It detects if the frontend is trying to save data to a column that doesn't exist yet and **creates it automatically** on the fly.
- **Why?** Eliminates the need for manual SQL scripts or fix links.
- **Support**: Automatically handles text, integers, and boolean types.

### 2. Manual Fix Utility
If you prefer to update the database manually or ensure specific columns are created, use the utility scripts:
- **URL (Column type)**: `http://localhost:8000/fix_type_column.php` (Fixes "Failed to load SBCs")
- **URL (Column description)**: `http://localhost:8000/fix_sbc_db.php`
- **What it does**: Adds mission columns to `call_servers` (`type`) and `sbcs` (`description`).

### 3. SQL Syntax Safety & Backticks
All queries use backticks (`` ` ``) for all column names.
- **Why?** Prevents conflicts with MariaDB reserved keywords (like `name`).

### 4. Automatic Type Conversion
Boolean values (`true/false`) are auto-converted to integers (`1/0`) for database compatibility.
