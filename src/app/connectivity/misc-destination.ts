...
export class MiscDestinationComponent implements OnInit {
    ...
    getEmptyForm() {
        return { id: null, description: '', dial: '' };
    }

    loadItems() {
        this.isLoading = true;
        this.http.get<any>(`${environment.apiUrl}/misc-destinations`).subscribe({
    ...
    save() {
        if (!this.form.description || !this.form.dial) { this.showMessage('Description and Dial are required', 'error'); return; }
        const url = this.modalMode === 'create' ? `${environment.apiUrl}/misc-destinations` : `${environment.apiUrl}/misc-destinations/${this.form.id}`;
        ...
        Swal.fire({ title: 'Are you sure?', text: "You won't be able to revert this!", icon: 'warning', showCancelButton: true, confirmButtonText: 'Yes, delete it!' })
            .then((result) => {
                if (result.isConfirmed) {
                    this.http.delete(`${environment.apiUrl}/misc-destinations/${id}`).subscribe({
